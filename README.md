...
- **내 미술관 탭** — 자주 가는 미술관을 등록해두면 그 미술관들의 전시만 모아 보여줍니다. 처음 진입하면 전체 미술관 체크리스트가 뜨고, 하나라도 등록하면 전시 탭과 같은 카드형 목록(전체/전시중/전시예정 필터 포함)으로 바뀝니다. 상단 ⚙️ 버튼으로 등록 미술관을 언제든 추가/해제할 수 있습니다. 등록 목록은 기기의 `localStorage`에만 저장되며 기기 간 동기화는 되지 않습니다.
...
- **환기미술관/성곡미술관**: 두 사이트 다 HTTPS 인증서가 고장나 있어(자체서명/기본 nginx 플레이스홀더) 평문 HTTP로만 접속됩니다. 환기미술관은 워드프레스 REST API(`/wp-json/wp/v2/exhibitions`)로 목록을, 상세페이지 HTML에서 "전시기간" 필드를 따로 긁습니다. 성곡미술관은 REST API가 없어 목록·상세페이지를 모두 HTML로 긁습니다. 두 사이트의 포스터 URL도 http://라서 그대로 저장하면 https로 배포된 앱(GitHub Pages)에서 mixed content로 막혀 이미지가 안 뜹니다 — `data/exhibitions.json`에 저장할 때 `images.weserv.nl`(무료, 가입/키 불필요) 중계 URL로 바꿔서 https로 저장합니다.
