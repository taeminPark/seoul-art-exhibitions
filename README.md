...
**이미 후기(`reviewSummary`)가 있는 전시는 다시 수집하지 않고, 아직 없는 전시(= 새로 추가된 전시)만 수집합니다** — 스케줄을 얼마나 자주 돌리든 신규 전시 수만큼만 네이버 API를 호출하게 되어 과금 관점에서 가장 안전한 방식입니다. (`--force`로 전체 재수집 가능, `--limit N`으로 이번 실행 처리 건수 제한.)
